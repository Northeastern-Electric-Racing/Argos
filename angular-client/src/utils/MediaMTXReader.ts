...
const supportsNonAdvertisedCodec = (codec: string, fmtp: string | undefined) =>
  new Promise((resolve) => {
    ...
    pc.createOffer()
      .then((offer) => {
        ...
        if (!sections) {
          throw new Error('No Media Type Present');
        }
...
const unquoteCredential = (v: any) => JSON.parse(`"${v}"`);
...
const findFreePayloadType = (firstLine: string) => {
...
const enableStereoPcmau = (section: string) => {
...
const enableMultichannelOpus = (section: string) => {
  const lines = section.split('\r\n');
...
const enableL16 = (section: string) => {
  const lines = section.split('\r\n');
...
const enableStereoOpus = (section: string) => {
  ...
  for (let i = 0; i < lines.length; i++) {
    if (lines[i].startsWith('a=rtpmap:') && lines[i].toLowerCase().includes('opus/')) {
      [opusPayloadFormat] = lines[i].slice('a=rtpmap:'.length).split(' ');
      break;
    }
  }
...
const editOffer = (sdp: string | undefined, nonAdvertisedCodecs: string | string[]) => {
  if (!sdp) return sdp;
...
const generateSdpFragment = (od: { iceUfrag: string; icePwd: string; medias: any }, candidates: any) => {
...
export default class MediaMTXWebRTCReader {
  ...
  private nonAdvertisedCodecs = [] as string[];
  ...
  private setupPeerConnection = (
    ...
    this.pc.onicecandidate = (evt: any) => this.onLocalCandidate(evt);
  ...
  private onLocalCandidate = (evt: { candidate: null }) => {
    if (this.state !== 'running') {
      return;
    }
  ...
  private sendLocalCandidates = (candidates: any[]) => {
    fetch(this.sessionUrl!, {
      method: 'PATCH',
      headers: {
