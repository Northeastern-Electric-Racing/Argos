...
- **`code-review` is not vendored.** Argos already has a `/code-review`; `implement` uses it. `implement`'s commit step uses Argos's `/commit` convention, not a bare commit.

## Consequences

- New skills `.claude/skills/wayfinder/`, `.claude/skills/implement/`, and `.claude/skills/research/` (research adopted separately in #689). `implement` drives `tdd` → `code-review` → `commit`; it does not push or open PRs.
